...
- **SSH session spawn must not use Child.run**: `createSshSession` and `bridge_create_ssh_shell` MUST use `Child.init` + `spawn` + `wait` with `stdout_behavior = .Ignore` and `stderr_behavior = .Ignore`. Do NOT use `Child.run` — it creates stdout/stderr pipes that `tmux new-session -d` can leak to the long-running SSH child process, causing `Child.run` to block forever reading the pipe and freezing the UI. Direct `ssh` commands (probe, kill, remote tmux) are fine with `Child.run` since SSH exits after the command completes.
